...
class ProactiveFollowupService:
    ...
    async def list_followups(
        ...
        *,
        ...
        followups.sort(key=self._sort_key)
    ...
    async def _build_followup_item(
        self,
        *,
        session_id: str,
        ...
        projection, events = await asyncio.gather(
            self._stream_service.project_stream(
                stream_id=session_id,
                projector_name="session-runtime",
                projector_version="v1",
            ),
            self._stream_service.read_stream(stream_id=session_id),
        )
        ...
        selected_matrix_candidate = self._selected_matrix_candidate(
        ...
        latest_proactive_scheduling_event = self._latest_event(
            events,
            event_type=PROACTIVE_SCHEDULING_UPDATED,
        )
        latest_proactive_orchestration_event = self._latest_event(
            events,
            event_type=PROACTIVE_ORCHESTRATION_UPDATED,
        )
        latest_proactive_actuation_event = self._latest_event(
            events,
            event_type=PROACTIVE_ACTUATION_UPDATED,
        )
        latest_proactive_progression_event = self._latest_event(
            events,
            event_type=PROACTIVE_PROGRESSION_UPDATED,
        )
        latest_proactive_dispatch_gate_event = self._latest_event(
            events,
            event_type=PROACTIVE_DISPATCH_GATE_UPDATED,
        )
        latest_proactive_dispatch_feedback_event = self._latest_event(
            events,
            event_type=PROACTIVE_DISPATCH_FEEDBACK_ASSESSED,
        )
        latest_proactive_stage_controller_event = self._latest_event(
            events,
            event_type=PROACTIVE_STAGE_CONTROLLER_UPDATED,
        )
        latest_proactive_line_controller_event = self._latest_event(
            events,
            event_type=PROACTIVE_LINE_CONTROLLER_UPDATED,
        )
        latest_session_event = self._latest_event(
            events,
            event_type=SESSION_STARTED,
        )
        latest_user_event = self._latest_event(events, event_type=USER_MESSAGE_RECEIVED)
        latest_assistant_event = self._latest_event(
            events,
        ...
        if directive_status == "ready" and bool(directive.get("eligible")):
            ...
            while current_stage_index <= max_dispatch_count:
                ...
                current_stage_directive = self._stage_entry(
                    proactive_orchestration_plan.get("stage_directives"),
                    current_stage_label,
                )
                current_stage_actuation = self._stage_entry(
                    proactive_actuation_plan.get("stage_actuations"),
                    current_stage_label,
                )
                current_stage_progression = self._stage_entry(
                    proactive_progression_plan.get("stage_progressions"),
                    current_stage_label,
                )
                current_stage_guardrail = self._stage_entry(
                ...
                due_at, expires_at, schedule_reason = self._apply_matrix_learning_spacing(
                    due_at=due_at,
                ...
                due_at, expires_at, schedule_reason = self._apply_stage_guardrail(
                    due_at=due_at,
                    expires_at=expires_at,
                    schedule_reason=schedule_reason,
                ...
                due_at, expires_at, schedule_reason = self._apply_stage_controller(
                    due_at=due_at,
                    expires_at=expires_at,
                    schedule_reason=schedule_reason,
                    window_seconds=window_seconds,
                ...
                due_at, expires_at, schedule_reason = self._apply_line_controller(
                    due_at=due_at,
                    expires_at=expires_at,
                    schedule_reason=schedule_reason,
                    window_seconds=window_seconds,
                ...
                due_at, expires_at, schedule_reason = self._apply_dispatch_gate(
                    due_at=due_at,
                    expires_at=expires_at,
                    schedule_reason=schedule_reason,
                    window_seconds=window_seconds,
    ...
    def _sort_key(self, item: dict[str, Any]) -> tuple[object, ...]:
        queue_status = str(item.get("queue_status") or "hold")
    ...
    def _apply_matrix_learning_spacing(
        ...
        *,
        ...
        selected_candidate = self._selected_matrix_candidate(reengagement_matrix_assessment)
