"""Internal benchmark runner for the phase-1 Chinese friend-chat profile."""
...
FRIEND_CHAT_ZH_RUNTIME_PROFILE = "friend_chat_zh_v1"
FRIEND_CHAT_ZH_SYSTEM_MODEL = "openai/llama-3-1-8b-instruct"
FRIEND_CHAT_ZH_BENCHMARK_PROVIDER = "minimax"
FRIEND_CHAT_ZH_BENCHMARK_MODEL = "M2-her"
FRIEND_CHAT_ZH_BENCHMARK_API_BASE = "https://api.minimax.io"
FRIEND_CHAT_ZH_PERSONA_FILE = "benchmarks/prompts/lin_xiaoyu_persona.md"


def build_friend_chat_zh_env(base_env: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    env["RELATIONSHIP_OS_LLM_MODEL"] = FRIEND_CHAT_ZH_SYSTEM_MODEL
    ...
    env["BENCHMARK_REPORT_TITLE"] = "RelationshipOS Friend Chat ZH Benchmark"
    env["BENCHMARK_REPORT_SUBTITLE"] = (
        "Internal phase-1 benchmark for long-form Chinese friend-chat continuity, "
        "persona stability, and restrained social-world control."
    )
    env["BENCHMARK_REPORT_PAGE_TITLE"] = "RelationshipOS Friend Chat ZH Benchmark"
    return env
...
def _write_failure_bundle(
    *,
    ...
    results = build_failed_benchmark_results(
        ...
        report_title=env.get("BENCHMARK_REPORT_TITLE", "RelationshipOS Friend Chat ZH Benchmark"),
        ...
        report_page_title=env.get(
            ...
            "RelationshipOS Friend Chat ZH Benchmark",
        ),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the internal Chinese friend-chat benchmark",
    )
    parser.add_argument("--base-url", default="http://127.0.0.1:8011")
    parser.add_argument("--output-dir", default="benchmark_results/friend_chat_zh_demo")
    parser.add_argument(
        "--latest-dir",
        default="benchmark_results/friend_chat_zh_demo/latest",
    )
    ...
    env = build_friend_chat_zh_env(base_env)
    if not env.get("BENCHMARK_CHAT_API_KEY"):
        raise SystemExit("BENCHMARK_CHAT_API_KEY is required for the internal MiniMax benchmark")
    if not env.get("RELATIONSHIP_OS_LLM_API_KEY"):
        raise SystemExit("RELATIONSHIP_OS_LLM_API_KEY is required for the system arm")
    ...
    try:
        ...
        print(f"Friend-chat zh benchmark complete: {bundle['html']}")
        ...
        print(f"Friend-chat zh benchmark failed but emitted scored bundle: {bundle['html']}")
    finally:
